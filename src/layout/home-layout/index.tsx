...
//  const navigate = useNavigate()
  ...
  const getCategoryName = (pathname:any) => {
    ...
    if (/^\/home\/brands\/\d+$/.test(pathname)) return "Brand Category"; 
  ...
  const drawer = (
    <div >
     <div className='w-full py-[1px] flex items-center justify-center'>
        <img className='w-[250px] h-[55px]' src={Logo} alt="logo" />
      </div>
      {/* <Divider /> */}
      <List className='bg-[#FFF] min-h-[90vh]' >
          {navList.map((el, index) => (
            <NavLink key={index} to={el.path} className={el.path === pathname ? "block bg-[rgb(76,129,205)] text-white duration-200    " :"  "}>
